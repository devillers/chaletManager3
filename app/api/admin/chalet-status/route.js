...
export async function POST(request) {
  ...
  if (!chaletId || !status) return NextResponse.json({ error: "Missing payload" }, { status: 400 });
  if (!ALLOWED_STATUSES.includes(status)) return NextResponse.json({ error: "Invalid status" }, { status: 400 });
  if (!mongoose.isValidObjectId(chaletId)) return NextResponse.json({ error: "Invalid identifier" }, { status: 400 });

  await connectToDatabase();
  const chalet = await Chalet.findById(chaletId);
  if (!chalet) return NextResponse.json({ error: "Not found" }, { status: 404 });

  chalet.status = status;
  chalet.publishedAt = status === "published" ? new Date() : chalet.publishedAt; // OK côté serveur
